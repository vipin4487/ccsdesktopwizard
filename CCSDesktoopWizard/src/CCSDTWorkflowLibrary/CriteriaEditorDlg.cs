    ...
    public class CriteriaEditorDlg : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (this.dgvCriteria.SelectedRows.Count > 0)
            {
                ...
                ArrayList dataSource = (ArrayList) this.dgvCriteria.DataSource;
                dataSource.RemoveAt(this.dgvCriteria.SelectedRows[0].Index);
                this.dgvCriteria.DataSource = null;
                this.dgvCriteria.DataSource = dataSource;
            }
        }
            ...
            TR_001A:
                while (true)
                {
                    if (enumerator.MoveNext())
                    {
                        ...
                        {
                            this.curActivityList.Add(current.Name);
                        }
                        else if (current is ValidateButton)
                        {
                            ValidateButton button = current as ValidateButton;
                            if ((button != null) && (button.OutputParameters != null))
                            {
                                foreach (ValidateOutputParameter parameter in button.OutputParameters)
                                {
                                    ...
                                    {
                                        this.curActivityList.Add(current.Name + "_" + parameter.ParameterName);
                                    }
                                }
                            }
                        }
        ...
        private void CriteriaEditorDlg_Load(object sender, EventArgs e)
        {
            ...
            this.AddGridColumns();
            if (ReferenceEquals(this.criteriaList, null))
            {
        ...
        {
            this.btnDelete.Enabled = !ReferenceEquals(this.dgvCriteria.SelectedRows, null) ? (this.dgvCriteria.SelectedRows.Count > 0) : false;
        }
        ...
        {
            get => 
                ((this.criteriaList == null) || (this.criteriaList.Count != 0)) ? this.criteriaList : null;
            set
            {
                if (value != null)
                {
